...
- **推荐文案**：为 ChatGPT、Gemini、Claude 和豆包添加右侧导航栏与消息大纲。快速定位历史提问，并支持悬浮预览。

### 商品描述 (Description)
> **官方要求**：概览段落 + 简要列出主要功能。说明用户为什么喜欢，避免拼写错误和重复性的无关关键词。

**概览（Overview）**：
随着与 ChatGPT、Gemini、Claude 和豆包（更多平台逐步接入中）等 AI 助手对话的深入，记录往往变得极长。回头寻找某轮特定提问或梳理上下文时，只能依赖低效的鼠标滚动，严重打断思考心流。
ChatDot 专为 AI 深度用户设计，能够在支持的 AI 对话网页右侧无缝注入一个轻量级、低打扰的**导航侧边栏**与**消息大纲面板**。通过它，您可以像查阅文档目录一样纵览所有对话节点，一键精准定位至任何一条记录。
...
- **原生级视觉融合**：支持浅色、深色与跟随系统主题，内置中文、English、日本語及한국어多语言设置。
- **多平台支持**：当前支持 ChatGPT、Gemini、Claude 和豆包（更多平台逐步接入中），并针对豆包反向滚动列表优化跳转定位。
...
| **宣传图块 (Small Promo)** | `440x280 px` | 使用清晰对比与明确边界，避免素材基调与产品视觉脱节。 | 采用与产品一致的浅色薄荷基调，突出 ChatDot Logo，并用真实箭头式导航栏元素表达“轻量导航”的实际体验。 |
| **主打图块 (Marquee Promo)** | `1400x560 px` | 设计尽量专业，图片缩小一半时仍清晰。**严禁带有“编辑精选/第一”等虚假声明**。 | 采用清新、低饱和的浅薄荷界面语言，右侧直接渲染扩展真实的箭头式侧边导航与预览效果，避免使用与产品不符的点状长条概念图。 |
