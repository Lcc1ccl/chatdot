...
## 版本与发布纪律
- `manifest.json` 的 `version` 是仓库版本真相；每次准备提交 PR 或生成提交前，必须先完成版本号递增。
- 默认递增规则：
  - 修补功能 / 缺陷修复：`+0.0.1`
  - 新功能：`+0.1.0`
- 若变更不属于修补或新功能，或需要跳版本、保留版本，必须先说明原因并获得确认。
- 版本号变化时，必须同步更新对外展示或依赖版本号的相关内容，避免仓库内版本口径不一致。
- 新功能若影响商店展示，必须同步更新商店资产相关文档与文案，至少包括 `docs/final/webstore-listing-plan.md`、`docs/final/store-listing-i18n.md`、`docs/plans/plan.md`。
- 当功能影响商店展示时，交付说明里必须显式提醒“商店资产素材已更新，请前往后台刷新相关资产”。
- 单个会话的代码变更完成后，在请求用户验证前，必须同步触发 `release.zip` 打包操作，并以该产物作为验证对象之一。
...
- 涉及页面、交互、注入逻辑、设置联动或发布物变更时，验证阶段必须同步输出当前变动的真机验证流程，包括验证设备、浏览器、目标页面、操作步骤、预期结果、实际结果，以及是否已生成 `release.zip`。
